...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, m_limit_descendants, m_limit_descendant_size, errString)) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/baddcoin-dev/2018-November/016518.html
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params &consensusParams) {
    CAmount nSubsidy;
    if (nHeight == 10 || nHeight == 50 || nHeight == 70 || nHeight == 90 || nHeight == 110 || nHeight == 1800)  {
        nSubsidy = 250000 * COIN;   // premine 6x250000 coins
    } else {
        if (nHeight <= 2000) {
            nSubsidy = 100 * COIN ;  // small mining to carry out the transactions
        } else {
...
if (block.GetHash() != chainparams.GetConsensus().hashGenesisBlock)
{
    ...
    if (block.GetHash() == chainparams.GetConsensus().hashGenesisBlock) {
        ...
        //return true; //make genesis spendable
    }
...
{
    //LogPrintf("DEBUG error detail:pindexPrev(validation): %s:",pindexPrev);
    assert(pindexPrev != nullptr);
    const int nHeight = pindexPrev->nHeight + 1;
    //LogPrintf("DEBUG error detail:nHeight %s:",nHeight);
    ...
    // [- DEBUGGING:
    const int nxtWork = GetNextWorkRequired(pindexPrev, &block, consensusParams);
    LogPrintf("\nDEBUG detail (badd-diffbits): nHeight=%s: nBits=%s: pindexPrev=%s\n", nHeight, block.nBits, pindexPrev);
    LogPrintf("DEBUG detail (badd-diffbits): nxtWork: %s\n", nxtWork);
    //I can see mining genesis + 1 block after the difficulty nbits and next work don't match - very slightly off
    //--> nHeight=1: nBits=504365055: pindexPrev=0x7f9944002340DEBUG detail (badd-diffbits): nxtWork: 504365040
    //--> nHeight=2: nBits=504365055: pindexPrev=0x7f9944000d20DEBUG detail (badd-diffbits): nxtWork: 504365055
    // -]

    if(nHeight == 0 || nHeight == 1){ //fix by skipping these two
    } else {
...
if(nHeight == 0){
	//LogPrintf("DEBUG (Genesis) detail: nHeight: %s:",nHeight);
}else{
    if (nHeight >= consensusParams.CSVHeight) {
 	//LogPrintf("DEBUG detail: nHeight: %s:",nHeight);
        assert(pindexPrev != nullptr);
