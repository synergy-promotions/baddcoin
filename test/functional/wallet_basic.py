...
class WalletTest(BaddcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 21 BADD from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 100 badd in block rewards plus fees, but
        ...
        # Send 10 BADD with subtract fee from amount
        ...
        # Sendmany 10 BADD with subtract fee from amount
        ...
        # Sendmany with explicit fee (BADD/kB)
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid Baddcoin address
            assert_raises_rpc_error(-5, "Invalid Baddcoin address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid Baddcoin address
            assert_raises_rpc_error(-5, "Invalid Baddcoin address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid Baddcoin address or script", self.nodes[0].importaddress, "invalid")
            ...
            # send with explicit badd/kb fee
            self.log.info("test explicit fee (sendtoaddress as badd/kb)")
            self.nodes[0].generate(1)
            self.sync_all(self.nodes[0:3])
