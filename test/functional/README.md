...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The Travis linter also checks this, but [possibly not in all cases](https://github.com/synergy-promotions/baddcoin/pull/14884#discussion_r239585126).
...
- Use a module-level docstring to describe what the test is testing, and how it
  is testing it.
- When subclassing the BaddcoinTestFramework, place overrides for the
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to control whether
  ...
  has 25 mature blocks (25x50=1250 BADD) in its wallet.
...
with the baddcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a baddcoind.  `P2PInterface`
contains the higher level logic for processing P2P payloads and connecting to
the Baddcoin Core node application logic. For custom behaviour, subclass the
...
The [`TestShell`](test-shell.md) class exposes the BaddcoinTestFramework
...
Taken from the [python-baddcoinrpc repository](https://github.com/jgarzik/python-baddcoinrpc).
...
Test objects for interacting with a baddcoind node over the p2p interface.

#### [script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-baddcoinlib)
