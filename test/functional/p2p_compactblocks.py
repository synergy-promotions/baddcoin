...
# TestP2PConn: A peer we use to send messages to baddcoind, and store responses.
...
class CompactBlocksTest(BaddcoinTestFramework):
    ...
    # This test actually causes baddcoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that baddcoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for baddcoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # baddcoind will not send blocktxn responses for blocks whose height is
