*After branching off for a major version release of Baddcoin Core, use this
...
https://github.com/baddcoin-core/baddcoin-devwiki/wiki/
...
Baddcoin Core version *version* is now available from:

  <https://baddcoin.org/bin/baddcoin-core-*version*/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://baddcoin.org/en/list/announcements/join/>
...
installer (on Windows) or just copy over `/Applications/Baddcoin-Qt` (on Mac)
or `baddcoind`/`baddcoin-qt` (on Linux).

Upgrading directly from a version of Baddcoin Core that has reached its EOL is
possible, but it might take some time if the data directory needs to be migrated. Old
wallet versions of Baddcoin Core are generally supported.
...
Baddcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Baddcoin
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Baddcoin Core on
unsupported systems.

From Baddcoin Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Baddcoin Core does not yet change appearance
...
RPC commands have been updated to include two new fee estimation methods "BADD/kB" and "sat/B".
...
- A new `baddcoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the
  former `generate` RPC. See the help for details. (#19133)

- The `baddcoin-cli -getinfo` command now displays the wallet name and balance for
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `baddcoin-wallet` tool which performs the salvage
...
Users who already have a Baddcoin Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which is the default type of wallet) will
behave as they did in previous versions of Baddcoin Core.
...
Default Wallet created upon first startup of Baddcoin Core will be a Legacy Wallet.
...
The change to using descriptors changes the default derivation paths used by Baddcoin Core
...
[Transifex](https://www.transifex.com/baddcoin/baddcoin/).
