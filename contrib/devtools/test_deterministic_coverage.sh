...
TEST_BADDCOIN_BINARY="src/test/test_baddcoin"
...
if [[ ! -e ${TEST_BADDCOIN_BINARY} ]]; then
    echo "Error: Executable ${TEST_BADDCOIN_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_BADDCOIN_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
