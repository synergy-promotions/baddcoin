...
(define* (make-baddcoin-cross-toolchain target
                                  #:key
...
desirable for building Baddcoin Core release binaries."
  ...
  (let ((target (getenv "HOST")))
    (cond ((string-suffix? "-mingw32" target)
          ...
          ((string-contains target "riscv64-linux-")
           (list (make-baddcoin-cross-toolchain "riscv64-linux-gnu"
                                               #:base-gcc-for-libc gcc-7)))
          ((string-contains target "-linux-")
           (list (make-baddcoin-cross-toolchain target)))
          (else '())))))
